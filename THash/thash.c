...
SList *thash_insertar(void *dato, SList * tablaHash, FuncionHash hash,
                      FuncionComparar comparar, FuncionDestruir destruir) {
  int key = hash(dato);
  tablaHash[key] = slist_insertar(tablaHash[key], dato, comparar, destruir);
  return tablaHash;
}

void *thash_buscar(void *dato, SList * tablaHash, FuncionHash hash,
