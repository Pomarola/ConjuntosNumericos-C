...
void operacion_dos_conjuntos(char *primerTermino, char *segundoTermino,
                             SList * tablaHash) {
  ...
  primerConjunto =
      ((Conjunto)
       thash_buscar(primerBuffer, tablaHash, hash_str,
                    conjunto_comparar_string));
  segundoConjunto =
      ((Conjunto)
       thash_buscar(segundoBuffer, tablaHash, hash_str,
                    conjunto_comparar_string));
  ...
  if (primerConjunto && segundoConjunto) {
    ...
    case '|':
      tablaHash =
          thash_insertar(conjunto_union
                         (primerTermino, primerConjunto, segundoConjunto),
    ...
    case '&':
      tablaHash =
          thash_insertar(conjunto_interseccion
                         (primerTermino, primerConjunto, segundoConjunto),
                         tablaHash, hash_conjunto, conjunto_comparar_nombre,
                         dlist_destruir);
      break;
...
int descomponer_entrada(char *entrada, char *primerTermino,
                        char *segundoTermino) {
...
void realizar_operacion(char *primerTermino, char *segundoTermino,
                        SList * tablaHash) {
  Conjunto conjunto;
  ...
  if (segundoTermino[i] == '{') {
    if (segundoTermino[i + 1] >= 'a' && segundoTermino[i + 1] <= 'z') {
      ...
      if (conjunto)
        tablaHash =
            thash_insertar(conjunto, tablaHash, hash_conjunto,
                           conjunto_comparar_nombre, dlist_destruir);
    } else {
    ...
    conjunto =
        thash_buscar(buffer, tablaHash, hash_str, conjunto_comparar_string);
    if (conjunto)
      tablaHash =
          thash_insertar(conjunto_complemento(primerTermino, conjunto),
                         tablaHash, hash_conjunto, conjunto_comparar_nombre,
                         dlist_destruir);
    else
...
int main() {
  ...
  while (strcmp(leer_entrada(entrada), "salir\0") != 0) {
    ...
    switch (codigo) {
    case 1:
      dlist_imprimir(thash_buscar
                     (segundoTermino, tablaHash, hash_str,
                      conjunto_comparar_string), imprimir_intervalo);
      break;
